...
window.PageModules["new-visit"] = {
  init() {
    ...
    UI.$("#visitForm").addEventListener("submit", (e) => {
      e.preventDefault();
      ...
      if (mode === "lead") {
        ...
        const newId = Store.uid("c");
        const current = Store.getState();
        current.clients.unshift({
          id: newId,
          name,
          company: name,
          contact: fd.get("leadContact"),
          phone: fd.get("leadPhone"),
          email: "",
          city: fd.get("leadCity") || "Não informado",
          state: fd.get("leadState") || "CE",
          segment: "Não informado",
          category: "Outro",
          status: "Lead",
          lat: geo.lat,
          lng: geo.lng,
          notes: "Lead captado durante visita.",
          needs: [],
      ...
      UI.toast("Visita salva no histórico.");
